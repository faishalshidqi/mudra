...
model_Bisindo = load_model('model_Bisindo_FIX.h5')
model_SIBI = load_model('model_SIBI.h5')
model_American = load_model('model_Bisindo_1.h5')
...
def trigger(model,image_path):
    ...
    input_IMG = np.array([[[pergelangan_X], [pergelangan_Y], [pergelangan_Z],
                        ...
                        [kelingking_DipX], [kelingking_DipY], [kelingking_DipZ],
                        [kelingking_TipX], [kelingking_TipY], [kelingking_TipZ]]])
    if np.all(input_IMG == 0) or np.all(input_IMG == None):
        response_dict = {
            'status': 'error',
...
def predict():
    if request.method == 'POST':
        try:
            ...
            flash("Terjadi kesalahan dalam memproses permintaan.")
            return redirect(url_for('predict'))

    return render_template('telunjuk1.html')
